...
ShmSemaphore::ShmSemaphore(const std::string& sName)
    : name_(sName), ptr_(nullptr), shm_id_(-1), sem_id_(nullptr), m_nSize(0) {
  /**
   * Semaphore open
   */
...
bool ShmSemaphore::Create(size_t nSize, int mode /*= READ_WRITE*/) {
  m_nSize = nSize;
  shm_id_ = shm_open(name_.c_str(), O_CREAT | mode, S_IRWXU | S_IRWXG);
  if (shm_id_ < 0) {
    switch (errno) {
      case EACCES:
...
bool ShmSemaphore::Attach(int mode /*= A_READ | A_WRITE*/) {
  ...
  ptr_ = mmap(NULL, m_nSize, mode, MAP_SHARED, shm_id_, 0);
  if (ptr_ == nullptr) {
...
bool ShmSemaphore::Detach() { munmap(ptr_, m_nSize); }

bool ShmSemaphore::Lock() { sem_wait(sem_id_); }

bool ShmSemaphore::UnLock() { sem_post(sem_id_); }
